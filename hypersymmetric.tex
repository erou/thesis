...
\[
  \Phi(x_1, \dots, x_s) =
  \sum_{j=1}^t\ps{a_j}{x_1}\dots\ps{a_j}{x_s}w_j.
\]
...
\item Algebras of truncated polynomials $\A = \mathbb{F}_q[T]/(T^k)$. In this
  ...
  \begin{array}{cccc}
    \tau: & \A & \to & \K \\
    & \sum_{j=0}^{k-1}x_j T^j & \mapsto & x_{k-1}
  \end{array}
\]
and
\[
  \ps{x}{y} = \tau(xy).
\]
Indeed, if $x=\sum_{j=0}^{k-1}x_jT^j$ and $y=\sum_{j=0}^{k-1}y_jT^j$, we have
\[
  \tau(xy) = x_0y_{k-1} + x_1y_{k-2} + \dots + x_{k-1}y_0,
\]
thus $\ps{\cdot}{\cdot}$ is a non-degenerate bilinear form.
\end{itemize}
...
non-degenerate bilinear form, written as a scalar product $\ps{\cdot}{\cdot}$,
...
\[
  \E_1^{\left\{ (\lambda_1, a_1) \right\}} = \left\{
    a\in\E_1\mid\exists\lambda\in\K,\,\rank(b_1-\lambda f(a)) <\rank(b_1)
\right\}.
\]
...
into $r_1$ bilinear forms of rank $1$, \ie finds decompositions with exactly
...
bounds from \cite[Thm. 2.2]{BCPRRR21} and \cite{BDEZ12},
this gives 
\[
...
know~\cite[Table~2]{BCPRRR21} that $\musym_3(7)\leq19$, so we have
\[
...
\[
  \E = \bigcup_{i=1}^k\E_i
\]
be subsets of $\A$. Now, for each $a\in \E$, we write 
\[
  \bff(a) = a\otimes \f(a),
\]
...
2.2]{BCPRRR21}. In order to prove that
...
\begin{itemize}
  ...
  \item if $\A = \mathbb{F}_{q}[T]/(T^k)$ and $x=\sum_{j=0}^{k-1}x_jT^j$, then
    $\tau(x) = x_{k-1}$ is the map sending an element to its leading coefficient.
\end{itemize}
