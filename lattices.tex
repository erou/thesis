...
\[
  a\,|\,b
\]
two integers, we would like to manage more than $2$ finite fields
...
\[
  k\subset K\subset L
\]
with $k\emb K$ and $k\emb L$, as in Figure~\ref{fig:incomplete}. The fundations
...
\begin{prop}[{\cite[Thm.~2.1]{BCS97}}]
  ...
  \[
    k\subset K\subset L
  \]
  be three finite fields extensions of $\mathbb{F}_p$, with respective
...
\begin{prop}[{\cite[Thm.~2.2]{BCS97}}]
  ...
  \[
    k\subset K\subset L
  \]
  three finite fields extensions of $\mathbb{F}_p$, with respective
  cardinalities $p^l$, $p^m$ and $p^n$. Assume that $k\emb K$ and $k\emb L$,
...
as a consequence we were only able to implement Bosma-Canon-Steel algorithm for
the type \texttt{FqNmodFiniteField}. This type contains
\begin{itemize}
  \item the characteristic $p$ (that fits in $64$ bits);
  \item the irreducible polynomial used to define the field (of type
    \texttt{nmod\_poly\_t});
  \item various technical informations that are stored for performance;
  \item two dictionaries containing informations about the lattice of
...
such as intersections computations and recursive computations of other
