...
\paragraph{Main algorithms.} The embedding algorithm thus follows these steps
(alongside trivial verifications such as checking if the embedding makes sense,
checking if an embedding already exists, and so on)
and there are three main algorithms, \texttt{intersections},
\texttt{find\_morphism} and \texttt{transitive\_closure} corresponding to the
three steps.
