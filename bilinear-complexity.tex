...
exists for symmetric bilinear maps, but it is indeed true, thus
% TODO: find a reference for that fact
...
\]
We have no example of algebra $\A = \mathbb{F}_{q^k}[T]/(T^l)$ where the
quantities $\mu_q(k, l)$ and $\musym_q(k, l)$ are different.
